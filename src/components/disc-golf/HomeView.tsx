      ...
      if (error) {
        if (error.includes('Database error')) {
          ...
          fetch('/api/setup/fix-signup')
            .then(r => r.json())
            .then(data => { if (data.fixSql) setFixSql(data.fixSql); })
            .catch(() => {});
        }
        toast.error('Rekisteröinti epäonnistui', { description: error.includes('Database error') ? 'Tietokantavirhe — katso korjausohjeet alla' : error });
              ...
              <CardContent className="p-6 space-y-4">
                ...
                {authMode === 'login' ? (
                  ...
                  <form onSubmit={handleRegister} className="space-y-3">
                    ...
                    {signupBroken && (
                      <div className="rounded-lg border border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950/30 p-3 space-y-2">
                        <p className="text-sm font-semibold text-red-800 dark:text-red-300">⚠️ Tietokantavirhe</p>
                        <p className="text-xs text-red-700 dark:text-red-400">Rekisteröityminen ei toimi tietokantatriggerin virheen vuoksi. Korjaa ajamalla SQL Supabasen SQL Editorissa.</p>
                        <ol className="text-xs text-red-700 dark:text-red-400 space-y-1 list-decimal pl-4">
                        ...
                        {fixSql && (
                          <div className="relative">
                            <pre className="bg-muted/80 p-2 rounded-md text-[9px] font-mono overflow-x-auto max-h-32 overflow-y-auto whitespace-pre-wrap">{fixSql}</pre>
                            <Button variant="outline" size="sm" className="absolute top-1 right-1 h-6 text-[10px] px-2" onClick={() => { navigator.clipboard.writeText(fixSql); setFixSqlCopied(true); toast.success('SQL kopioitu!'); setTimeout(() => setFixSqlCopied(false), 2000); }}>
                              {fixSqlCopied ? '✓' : 'Kopioi'}
                            </Button>
                          </div>
                        )}
